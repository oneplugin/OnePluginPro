...
use oneplugin\onepluginpro\models\OnePluginProOptimizedImage;
...
class OptimizeImageJob extends BaseJob
{
    ...
    {
        return Craft::t('one-plugin-pro', 'Generating Optimized images.');
    }
    ...
    public function execute($queue): void
    {
        ...
        if( $asset ){
            $assets = OnePluginProOptimizedImageRecord::find()->where(['assetId' => $this->assetId] )->all();
            ...
            Craft::$app->db->createCommand()
                    ->upsert(OnePluginProOptimizedImageRecord::tableName(), [
                        'content' => $json,
                        'assetId' => $asset->getId()
                    ], true, [], true)
                    ->execute();
            ...
            OnePluginProOptimizedImageRecord::find()->where(['assetId' => $this->assetId])->one()->delete();
        }
    }
    ...
    private function generateOptimizedImage(Asset $asset, $force){
        ...
        $model = new OnePluginProOptimizedImage('');
